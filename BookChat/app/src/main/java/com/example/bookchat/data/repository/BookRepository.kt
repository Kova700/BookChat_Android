...
class BookRepository @Inject constructor() {
    ...
    suspend fun registerBookShelfBook(requestRegisterBookShelfBook: RequestRegisterBookShelfBook) {
        if (!isNetworkConnected()) throw NetworkIsNotConnectedException()
        val response =
            App.instance.bookChatApiClient.registerBookShelfBook(requestRegisterBookShelfBook)
        when (response.code()) {
            200 -> {}
            else -> throw Exception(
                createExceptionMessage(
    ...
    suspend fun changeBookShelfBookStatus(book: BookShelfItem, readingStatus: ReadingStatus) {
        ...
        val response =
            App.instance.bookChatApiClient.changeBookShelfBookStatus(book.bookShelfId, requestBody)
        when (response.code()) {
            200 -> {}
            else -> throw Exception(
                createExceptionMessage(
    ...
    suspend fun checkAlreadyInBookShelf(book: Book): RespondCheckInBookShelf? {
        ...
        val response =
            App.instance.bookChatApiClient.checkAlreadyInBookShelf(book.isbn, book.publishAt)
        when (response.code()) {
            200, 404 -> {
